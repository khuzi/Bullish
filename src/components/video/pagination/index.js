...
export class Pagination extends React.Component {
    ...
    get hasPages(){
        return (!isNaN(this.state.pagesAmmount) && (this.state.pagesAmmount > 0))
        || (!isNaN(this.state.page_length) && (this.state.page_length > 0));
    }

    get disabled(){
        return this.state.loading || this.isLast || !this.hasPages;
    }
            ...
            case PAGINATION_TYPE.LAZY:
                return (
                    ...
                    className="lazy-pagination">
                        <button onClick={this.goNext} className="btn btn-dark btn-full" disabled={this.disabled}>More</button>
                    </nav>
                );
            case PAGINATION_TYPE.PAGINATION:
                return (
